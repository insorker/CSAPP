...
#if KALLOC_CHECK
		printf("FREE:  idx: %ld size: %ld address: %p\n",
			idx,
			hb_idx2size(idx),
			hb_idx2addr(hb, idx)
		);
...
int mm_init() {
  uintptr_t pmsize = MAX_HEAP;
	mem_sbrk(MAX_HEAP);

	// initialize global variable
	HB_number    = pmsize / (HB_WHOL_SIZE + sizeof(heap_block));
	HB_head_base = mem_heap_lo();
	HB_cont_base = mem_heap_lo() + HB_number * sizeof(heap_block);
#if KALLOC_CHECK
	printf("kinit:\nHB_number: %ld\nHB_head_base: %p\nHB_cont_base: %p\n",
	...
	if (!hb_check_size(size)) {
		...
		if (size <= HB_MAX) {
			...
			for (size_t i = 0; i < HB_number; i++) {
				...
				if (hb_idx) {
#if KALLOC_CHECK
					printf("ALLOC: idx: %ld size: %ld address: %p\n",
						hb_idx,
						hb_idx2size(hb_idx),
